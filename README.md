...
如果你希望把“报告问答”接到现成的 RAGFlow 知识库，而不是继续只使用本地 `LLMBackend`，可以开启可选的 RAGFlow QA 模式。

1. 在 RAGFlow 中准备一个可用的 chat assistant，并记下它的 `chat_id`
2. 获取 RAGFlow API Key
3. 启动本项目 API 时设置下面几个环境变量

```bash
TEXT_QA_PROVIDER=ragflow
TEXT_RAGFLOW_BASE_URL=http://127.0.0.1:9380
TEXT_RAGFLOW_API_KEY=<YOUR_RAGFLOW_API_KEY>
TEXT_RAGFLOW_CHAT_ID=<YOUR_RAGFLOW_CHAT_ID>

# 可选
TEXT_RAGFLOW_MODEL=ragflow          # 默认 ragflow，占位即可
TEXT_RAGFLOW_REFERENCE=true         # 向 RAGFlow 请求附带 reference 信息
TEXT_RAGFLOW_TIMEOUT_SECONDS=45     # 请求超时
```

说明：
- 当前集成点是“已完成分析报告”的 QA 面板，不会替换现有多 Agent 分析流水线。
- QA 请求会同时携带当前报告上下文，因此回答既能引用本次分析结果，也能利用 RAGFlow 已索引的外部知识。
- 如果 `TEXT_QA_PROVIDER` 不设置，系统仍然使用当前本地 LLM QA 逻辑。
...
TEXT_QA_PROVIDER=local           # QA 提供方：local | ragflow（默认 local）
```
