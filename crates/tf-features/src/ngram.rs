...
fn truncate_to_top_k(
    ...
    entries.select_nth_unstable_by(max_k - 1, |a, b| b.1.cmp(&a.1));
...
mod tests {
    ...
    fn test_char_bigrams_simple() {
        let result = char_ngrams("abc", 2, 200);
    ...
    fn test_word_bigrams() {
        let result = word_ngrams("hello world foo", 2, 200);
        assert_eq!(result.len(), 2);
    ...
    #[test]
    fn test_empty_input() {
        assert!(char_ngrams("", 2, 200).is_empty());
        assert!(word_ngrams("", 2, 200).is_empty());
    }

    #[test]
    fn test_n_larger_than_input() {
        assert!(char_ngrams("ab", 5, 200).is_empty());
        assert!(word_ngrams("hello", 3, 200).is_empty());
    }
